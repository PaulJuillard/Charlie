...
public class SimilarityBasedSearch {
	...
	public static double normalizedCrossCorrelation(int row, int col, double[][] pattern, double[][] image) {
		...
		// TODO almost done
		// assertions!
		// vérifier VARIANCE et ECART TYPE les carrés et tout et tout...
		double NCC = 0; //Normalized Cross Correlation
		
		double imageMean = windowMean(image); // image mean gray value
		double imageVariance = 0; // sum of the difference the image pixels' gray value and the image's mean gray value
		...
		for (int irow = 0; irow < pattern.length ; ++irow) {
			for (int icolumn = 0 ; icolumn < pattern[0].length ; ++ icolumn) {
				imageVariance += image[irow + row][icolumn + col] - imageMean;
				patternVariance += pattern[irow][icolumn] - patternMean;	
				covarianceSum += (image[irow + row][icolumn + col] - imageMean) * (pattern[irow][icolumn] - patternMean);
				}
		}
